...
def addtask(request):
    ...
    to_do_database = TodoDatabase.objects.create(owner=owner_var, title=title_var, description=desc_var, deadline=deadline_var)
    # to_do_database.save()
...
def deletetask(request):
    ...
    del_2 = TodoDatabase.objects.get(id=del_id, owner=request.user)
    del_2.delete()
    return redirect('todomain')
...
def details(request, pk):
    details1 = TodoDatabase.objects.get(id=pk, owner=request.user)
    context = {'tododetails': details1}
    return render(request, 'todoapp/details.html', context)


def update(request, pk):
    update1 = TodoDatabase.objects.get(id=pk, owner=request.user)
...
def updatetask(request, pk):
    updatetask1 = TodoDatabase.objects.get(id=pk, owner=request.user)
...
def signupaction(request):
    ...
    password_var = request.POST.get('password')
    User.objects.create_user(username=username_var, email=email_var, password=password_var)
    return redirect('authpage')
